...
comp_modrho = [modrho, modrho_sm, modrho_GO, modrho_GO_sm, modrho_GO_noVA, modrho_GO_noVA_sm];
...
variableNames_rho = {'modrho', 'modrho_sm', 'modrho_GO', 'modrho_GO_sm', 'modrho_GO_noVA', 'modrho_GO_noVA_sm'};
...
comp_vcv = [diag(modvcv), diag(modvcv_sm), diag(modvcv_GO), diag(modvcv_GO_sm), diag(modvcv_GO_noVA), diag(modvcv_GO_noVA_sm)];
...
variableNames_vcv = {'vcv', 'vcv_sm', 'vcv_GO', 'vcv_GO_sm', 'vcv_GO_noVA', 'vcv_GO_noVA_sm'};
...
for i = 1:36
    if i == 36
        Statistics_OffDiagonal_vcv{1,i} = [diag(modvcv,-i), diag(modvcv_sm,-i), diag(modvcv_GO_noVA,-i), diag(modvcv_GO_noVA_sm,-i)];
        continue;
    end
        comp_vcv = [diag(modvcv,-i), diag(modvcv_sm,-i), diag(modvcv_GO,-i), diag(modvcv_GO_sm,-i), diag(modvcv_GO_noVA,-i), diag(modvcv_GO_noVA_sm,-i)];
