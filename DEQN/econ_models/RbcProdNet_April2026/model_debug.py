"""
Debug version of model.py that adds NaN checking and diagnostics.
Use this temporarily to identify where NaNs originate.
"""
...
def check_nan(value, name, context=""):
    """Check for NaN/Inf and print diagnostic info if found."""
    has_nan = jnp.any(jnp.isnan(value))
    has_inf = jnp.any(jnp.isinf(value))
    
    def print_issue():
        print(f"\n{'='*60}")
        print(f"NUMERICAL ISSUE DETECTED: {name}")
        if context:
            print(f"Context: {context}")
        print(f"Has NaN: {has_nan}, Has Inf: {has_inf}")
        print(f"Shape: {value.shape if hasattr(value, 'shape') else 'scalar'}")
        print(f"Min: {jnp.nanmin(value)}, Max: {jnp.nanmax(value)}")
        print(f"Mean: {jnp.nanmean(value)}")
        # Show which elements are problematic
        if hasattr(value, 'shape') and value.size < 100:
            print(f"Values: {value}")
        print(f"{'='*60}\n")
    
    jax.lax.cond(
        has_nan | has_inf,
        lambda: jax.debug.callback(print_issue),
        lambda: None
    )
    return value


def check_positive(value, name, context=""):
    """Check that values are positive (for things that should be > 0)."""
    min_val = jnp.min(value)
    
    def print_issue():
        print(f"\n{'='*60}")
        print(f"NON-POSITIVE VALUE DETECTED: {name}")
        if context:
            print(f"Context: {context}")
...
class ModelDebug:
    """Debug version of RBC model with production networks."""
    ...
    def __init__(
        ...
        self.labels = [
            "Mining, Oil and Gas", "Utilities", "Construction", "Wood", "Minerals",
            "Primary Metals", "Fabricated Metals", "Machinery", "Computers", "Electrical",
            "Vehicles", "Transport", "Furniture", "Misc Mfg", "Food Mfg", "Textile",
            "Apparel", "Paper", "Printing", "Petroleum", "Chemical", "Plastics",
            "Wholesale Trade", "Retail", "Transp. and Wareh.", "Info", "Finance",
            "Real estate", "Prof/Tech", "Mgmt", "Admin", "Educ", "Health", "Arts",
    ...
    def expect_realization(self, state_next, policy_next):
        """Debug version with NaN checking."""
        ...
        # Check that denormalized policy is reasonable
        ...
        # Check positivity of variables that should be positive
        ...
        # Build expectation with checks
        A_power = A_next ** ((self.sigma_y - 1) / self.sigma_y)
        check_nan(A_power, "A^((sigma_y-1)/sigma_y)", "expect_realization")
    ...
    def loss(self, state, expect, policy):
        ...
        # Check all policy variables are positive
        ...
        # CRITICAL CHECK: MgUtCagg inner term
        MgUtCagg_inner = Cagg - self.theta * 1 / (1 + self.eps_l ** (-1)) * Lagg ** (1 + self.eps_l ** (-1))
        check_positive(MgUtCagg_inner, "MgUtCagg_inner (Cagg - theta*...)", "loss - THIS IS LIKELY THE CULPRIT")
        ...
        losses_array = jnp.concatenate(
            [C_loss, L_loss, K_loss, Pm_loss, M_loss, Mout_loss, Pk_loss, Iout_loss,
             Qrc_loss, Qdef_loss, Ydef_loss, Caggdef_loss, Laggdef_loss, Yaggdef_loss,
             Iaggdef_loss, Maggdef_loss],
            axis=0,
        )
        ...
        mean_accuracies_focs = jnp.array(
            [jnp.mean(1 - jnp.abs(C_loss)), jnp.mean(1 - jnp.abs(L_loss)), jnp.mean(1 - jnp.abs(K_loss)),
             jnp.mean(1 - jnp.abs(Pm_loss)), jnp.mean(1 - jnp.abs(M_loss)), jnp.mean(1 - jnp.abs(Mout_loss)),
             jnp.mean(1 - jnp.abs(Pk_loss)), jnp.mean(1 - jnp.abs(Iout_loss)), jnp.mean(1 - jnp.abs(Qrc_loss)),
             jnp.mean(1 - jnp.abs(Qdef_loss)), jnp.mean(1 - jnp.abs(Ydef_loss)), jnp.mean(1 - jnp.abs(Caggdef_loss)),
             jnp.mean(1 - jnp.abs(Laggdef_loss)), jnp.mean(1 - jnp.abs(Yaggdef_loss)),
             jnp.mean(1 - jnp.abs(Iaggdef_loss)), jnp.mean(1 - jnp.abs(Maggdef_loss))]
        )
        min_accuracies_focs = jnp.array(
            [jnp.min(1 - jnp.abs(C_loss)), jnp.min(1 - jnp.abs(L_loss)), jnp.min(1 - jnp.abs(K_loss)),
             jnp.min(1 - jnp.abs(Pm_loss)), jnp.min(1 - jnp.abs(M_loss)), jnp.min(1 - jnp.abs(Mout_loss)),
             jnp.min(1 - jnp.abs(Pk_loss)), jnp.min(1 - jnp.abs(Iout_loss)), jnp.min(1 - jnp.abs(Qrc_loss)),
             jnp.min(1 - jnp.abs(Qdef_loss)), jnp.min(1 - jnp.abs(Ydef_loss)), jnp.min(1 - jnp.abs(Caggdef_loss)),
             jnp.min(1 - jnp.abs(Laggdef_loss)), jnp.min(1 - jnp.abs(Yaggdef_loss)),
             jnp.min(1 - jnp.abs(Iaggdef_loss)), jnp.min(1 - jnp.abs(Maggdef_loss))]
        )

        return mean_loss, mean_accuracy, min_accuracy, mean_accuracies_focs, min_accuracies_focs

    # Copy remaining methods from original (sample_shock, mc_shocks, utility, etc.)
    ...
    def get_aggregates(self, policies_logdev):
        return {
            "Agg. Consumption": policies_logdev[11 * self.n_sectors + 2],
            "Agg. Labor": policies_logdev[11 * self.n_sectors + 3],
            "Agg. Capital": policies_logdev[11 * self.n_sectors + 6],
            "Agg. Output": policies_logdev[11 * self.n_sectors + 4],
            "Agg. GDP": policies_logdev[11 * self.n_sectors + 4],
            "Agg. Investment": policies_logdev[11 * self.n_sectors + 5],
            "Intratemporal Utility": policies_logdev[11 * self.n_sectors + 7],
        }
