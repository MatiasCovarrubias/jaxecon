...
| -------------- | ---------------------------------- | ------------------------- | ----------------- |
| `sigma_VA_agg` | Aggregate GDP volatility           | HP-filter log(sum(VA))    | Expenditure GDP from `sum(P.*(Q-M_out))` |
...
| `sigma_I_agg`  | Aggregate investment volatility    | HP-filter log(sum(Inv))   | Expenditure I from `sum(P.*I_out)` |
...
For aggregate `C`, `I`, and `GDP`, model moments are computed after Dynare simulation by reconstructing nominal expenditure aggregates in levels each period, then taking the standard deviation of log deviations from steady state:
