...
def create_batch_loss_fn_policies_precomputed_expects(econ_model):
...
def create_batch_loss_fn_expects_precompute_expects(econ_model, config):
  ...
  def batch_loss_fn(params, train_state, batch_obs, loss_rng):
    ...
    """Loss function of a batch of obs."""
    batch_policies, batch_expects = train_state.apply_fn(params, batch_obs, freeze_policies=True) # get the policies for the entire obs batch.
    ...
    # parallelize callculation of period_loss for the entire batch
    mean_losses, max_losses, mean_accuracies, min_accuracies = jax.vmap(period_loss)(batch_obs, batch_policies, batch_expects, jnp.stack(period_mc_rngs))
    mean_loss = jnp.mean(mean_losses)                   # average accross periods
    max_loss = jnp.max(max_losses)                   # max accross periods
...
def create_batch_loss_fn_simple_pretrain(econ_model, config):
...
def create_batch_loss_fn_pretrain(econ_model):
  ...
  def batch_loss_fn(params, train_state, batch_obs):
    ...
    # parallelize callculation of period_loss for the entire batch
    mean_losses, max_losses, mean_accuracies, min_accuracies = jax.vmap(period_loss)(batch_policies, batch_policies_parent)
    mean_loss = jnp.mean(mean_losses)                   # average accross periods
    max_loss = jnp.max(max_losses)                   # max accross periods
    mean_accuracy = jnp.mean(mean_accuracies)           # average accross periods
    min_accuracy = jnp.min(min_accuracies)              # min accross periods and across eqs within period
...
def create_batch_loss_fn_pretrain_pols_precompute_expects(econ_model):

  def batch_loss_fn(params, train_state, batch_obs):
    """Loss function of a batch of obs."""
    batch_policies, _ = train_state.apply_fn(params, batch_obs, freeze_expects=True) # get the policies for the entire obs batch.
    batch_policies_parent = jax.vmap(econ_model.policy_loglinear)(batch_obs)
...
def create_batch_loss_fn_pretrain_expects_precompute_expects(econ_model, config):
  ...
  def batch_loss_fn(params, train_state, batch_obs, loss_rng):
    ...
    """Loss function of a batch of obs."""
    _, batch_expects = train_state.apply_fn(params, batch_obs, freeze_policies=True) # get the policies for the entire obs batch.
    ...
    # parallelize callculation of period_loss for the entire batch
    mean_losses, max_losses, mean_accuracies, min_accuracies = jax.vmap(period_loss)(batch_obs, batch_expects, jnp.stack(period_mc_rngs))
    mean_loss = jnp.mean(mean_losses)                   # average accross periods
    max_loss = jnp.max(max_losses)                   # max accross periods
    mean_accuracy = jnp.mean(mean_accuracies)           # average accross periods
    min_accuracy = jnp.min(min_accuracies)              # min accross periods and across eqs within period
