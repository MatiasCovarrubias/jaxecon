...
% Options (change here)
save_exper_ind = 0; % 1 to save data  and graphs for the experiment.
...
% Cagg_stochss = exp(policies_ss(11*params.n_sectors+1));
% Lagg_stochss = exp(policies_ss(11*params.n_sectors+2));
% V_stochss = (1/(1-params.eps_c^(-1)))*(Cagg_stochss-params.theta*(1/(1+params.eps_l^(-1)))*Lagg_stochss^(1+params.eps_l^(-1)))^(1-params.eps_c^(-1))/(1-params.beta);
% Vc_stochss = (1/Cagg_ss)*(( V_stochss*(1-params.beta)*(1-params.eps_c^(-1)) )^(1/(1-params.eps_c^(-1))))  - 1;
...
% save ModStruct_temp par* policies_ss k_ss policies_stochss obs_stochss V_stochss Vc_stochss  N ax
...
for idx = 1:numel(sector_indices)
    ...
    % LOG-LINEAR IR
    shockssim_ir = zeros([params.simul_T, parn_sectors]); % we put the shock in steady_state not here
    dynare_simul = simult_(M_, options_, steady_state, oo_.dr, shockssim_ir, modorder);
    IRSLoglin = DynareProcessIR(dynare_simul,params,steady_state,labels,parn_sectors,k_ss, Cagg_ss, Lagg_ss,policies_ss);
    ...
    % save ModStruct_temp par* policies_ss k_ss simul_T shockssim_ir shocksim_0;
%     dynare determ_stochss_irs;
    dynare determ_irs;
    dynare_simul = Simulated_time_series.data';
    IRSDeterm = DynareProcessIR(dynare_simul,params,steady_state,labels,parn_sectors,k_ss, Cagg_ss, Lagg_ss,policies_ss);


    % COBB-DOUGLAS IR
    load('IRSLoglinCD.mat');
    
    % Produce & Save the graphs (if you want to graph less than three sets
    % of IRs, replace with [].
...
clearvars -except IRS* dynare_simul policies_ss parn_sectors params save*
IRS = struct();
...
if save_exper_ind == 1
    filename_Mod = strcat('output/IRS_', save_label, '.mat');
    save(filename_Mod, 'IRS');
end
