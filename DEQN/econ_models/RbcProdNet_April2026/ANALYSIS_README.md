...
- Welfare now also reports `C-recentered`, `L-recentered`, and `C and L recentered`, which shift the utility-relevant consumption path, labor path, or both so the sample mean matches the deterministic steady state.
...
- Welfare reporting now keeps the baseline nonlinear sample and, for each nonlinear DEQN experiment, also adds `C-recentered`, `L-recentered`, and `C and L recentered`. These are welfare-only counterfactuals that recenter the utility-relevant consumption path, labor path, or both so the relevant sample mean is zero.
