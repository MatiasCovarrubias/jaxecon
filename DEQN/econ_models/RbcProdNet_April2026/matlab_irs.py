...
def _extract_sectoral_dict(sectoral_data: Any) -> Optional[Dict[str, np.ndarray]]:
    ...
    parsed = {}
    for field in ("C_all", "L_all", "Iout_all", "Q_all", "Mout_all", "K_all"):
...
def _extract_aggregate_dict(aggregate_data: Any) -> Optional[Dict[str, np.ndarray]]:
    aggregate_dict = _coerce_mat_struct(aggregate_data)
    if not isinstance(aggregate_dict, dict):
        return None

    parsed = {}
    for field in ("C_utility", "C_exp", "I_exp", "GDP_exp", "L", "K"):
        arr = aggregate_dict.get(field)
        if arr is not None:
            parsed[field] = np.array(arr)
...
def _process_canonical_shock(shock_artifact: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for irf_data in entries:
        ...
        sectoral_second = _extract_sectoral_dict(irf_data.get("sectoral_secondorder"))
        if sectoral_second is not None:
            sector_entry["sectoral_second_order"] = sectoral_second
        ...
        aggregate_first = _extract_aggregate_dict(irf_data.get("aggregate_first_order"))
        if aggregate_first is not None:
            sector_entry["aggregate_first_order"] = aggregate_first

        aggregate_second = _extract_aggregate_dict(irf_data.get("aggregate_second_order"))
        if aggregate_second is not None:
            sector_entry["aggregate_second_order"] = aggregate_second
...
def _process_flat_format_shock(md_irs: Dict, shock_idx: int) -> Dict[str, Any]:
    ...
    for irf_data in irfs:
        ...
        for method_key, struct_key in [
            ("aggregate_first_order", "aggregate_first_order"),
            ("aggregate_second_order", "aggregate_second_order"),
            ("aggregate_perfect_foresight", "aggregate_perfect_foresight"),
        ]:
            aggregate_data = _extract_aggregate_dict(irf_data.get(struct_key))
            if aggregate_data is not None:
                sector_entry[method_key] = aggregate_data
...
def _process_new_format_shock(shock_result: Dict) -> Dict[str, Any]:
    ...
    for irf_data in irfs:
        ...
        if irs_first_order is not None:
            ...
            for method_key, struct_key in [
                ("sectoral_first_order", "sectoral_loglin"),
                ("sectoral_second_order", "sectoral_secondorder"),
                ("sectoral_perfect_foresight", "sectoral_determ"),
            ]:
                sectoral_data = irf_data.get(struct_key)
                if sectoral_data is not None:
...
def _trim_ir_series(raw: Any, max_periods: int, skip_initial: bool) -> Optional[np.ndarray]:
    if raw is None:
        return None
    arr = np.asarray(raw)
    if arr.size == 0:
    ...
    if arr.ndim > 1:
        arr = arr.ravel()
    return arr[1 : max_periods + 1] if skip_initial else arr[:max_periods]


def _get_matlab_ir_from_aggregate_blocks(
    ir_data: Dict[str, Any],
    sector_idx: int,
    analysis_var_name: str,
    max_periods: int = 100,
    ...
    field_name = AGGREGATE_BLOCK_VARIABLE_MAP.get(analysis_var_name)
    ...
    for key, data in ir_data.items():
        ...
        sector_data = data["sectors"][sector_idx]
        entry = {}
        for method_label, aggregate_key in [
            ("first_order", "aggregate_first_order"),
            ("second_order", "aggregate_second_order"),
            ("perfect_foresight", "aggregate_perfect_foresight"),
        ]:
            aggregate_block = sector_data.get(aggregate_key)
            if not isinstance(aggregate_block, dict):
                continue
            trimmed = _trim_ir_series(aggregate_block.get(field_name), max_periods, skip_initial)
            if trimmed is not None:
                entry[method_label] = trimmed
...
def _reaggregate_sectoral_ir(
    ...
    Pk_ergodic: Optional[np.ndarray],
    state_ss: Optional[np.ndarray],
    analysis_var_name: str,
    n_sectors: int,
    skip_initial: bool = True,
    max_periods: int = 100,
    ...
    Args:
        sectoral_data: Dict with sector-level log-deviation blocks (each n_sectors x T)
        ...
        Pk_ergodic: Fixed ergodic capital prices in levels (n_sectors-vector)
    ...
    if analysis_var_name in AGGREGATE_VARIABLE_SECTORAL_MAP:
        ...
        if field == "C_all":
            ...
            X_ss = ps_levels[n : 2 * n]
            weights = None
        elif field == "Iout_all":
            X_ss = ps_levels[7 * n : 8 * n]
            weights = P_ergodic
        elif field == "K_all":
            if Pk_ergodic is None or state_ss is None:
                return None
            X_ss = np.exp(state_ss[:n])
            weights = Pk_ergodic
        else:
            return None
        ...
        if weights is None:
            agg_t = np.sum(X_levels, axis=0)
            agg_ss = np.sum(X_ss)
        else:
            agg_t = weights @ X_levels
            agg_ss = weights @ X_ss
...
def get_matlab_ir_fixedprice(
    ...
    Args:
        ...
        state_ss: MATLAB steady-state states in log
        n_sectors: Number of sectors
        max_periods: Maximum number of periods to return
    ...
    for key, data in ir_data.items():
        ...
        for method_label, sectoral_key in [
            ...
            ("second_order", "sectoral_second_order"),
