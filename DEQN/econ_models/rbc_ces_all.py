...
class RbcCES_fixL:
    ...
    def loss(self, obs, expect, policy):
        ...
        Pkmodel = P * (1 - self.phi * (Inv / K - self.delta)) ** (-1)
        K_tplus1_def = (1 - self.delta) * K + Inv - (self.phi / 2) * (Inv / K - self.delta) ** 2 * K
        Ydef = A * (
            self.alpha ** (1 / self.sigma_y) * K ** ((self.sigma_y - 1) / self.sigma_y)
            + (1 - self.alpha) ** (1 / self.sigma_y) * L ** ((self.sigma_y - 1) / self.sigma_y)
        ) ** (self.sigma_y / (self.sigma_y - 1))
        ...
        L_loss = L / Lss - 1
        K_loss = Pk / MPK - 1
        Inv_loss = Pk / Pkmodel - 1
        P_loss = Y / (C + Inv) - 1
        Pk_loss = K_tplus1 / K_tplus1_def - 1
        Y_loss = Y / Ydef - 1

        losses_array = jnp.array([C_loss, L_loss, K_loss, Inv_loss, P_loss, Pk_loss, Y_loss])
        mean_loss = jnp.mean(losses_array**2)
    ...
    def get_aggregates(self, simul_policies):
        ...
        L = simul_policies[:, 1]
        K = simul_policies[:, 2]
        Inv = simul_policies[:, 3]
        Y = simul_policies[:, 6]
        aggregates = {"C": C, "L": L, "K": K, "Inv": Inv, "Y": Y}
        return aggregates


class RbcCES_fixL_short:
    """A JAX implementation of an RBC model."""
    ...
    def step(self, obs, policy, shock):
        ...
        a = obs_notnorm[1]  # a_{t}
        a_tplus1 = self.rho * a + self.shock_sd * shock[0]  # recover a_{t+1}
        Inv = policy[0] * jnp.exp(self.policies_ss[3])  # multiply by stst pols in level
    ...
    def expect_realization(self, obs_next, policy_next):
        ...
        Inv_next = policy_next[0] * jnp.exp(self.policies_ss[3])
        ...
        expect_realization = P_next * A_next ** (1 - self.sigma_y ** (-1)) * (self.alpha * Y_next / K_next) ** (
            1 / self.sigma_y
        ) + Pk_next * ((1 - self.delta) + self.phi / 2 * (Inv_next**2 / K_next**2 - self.delta**2))
