...
@#for j in 1:n_sectors
    ...
    exp(p_@{j}) = (exp(cagg)-theta*(1/(1+eps_l^(-1)))*exp(lagg)^(1+eps_l^(-1)))^(-eps_c^(-1)) * (exp(cagg) * xi_@{j}/exp(c_@{j}))^(1/sigma_c);    
    (exp(cagg)-theta*(1/(1+eps_l^(-1)))*exp(lagg)^(1+eps_l^(-1)))^(-eps_c^(-1))* theta * exp(lagg)^(eps_l^(-1))*(exp(l_@{j})/exp(lagg))^(1/sigma_l) = (exp(p_@{j})) * (exp(a_@{j}))^((sigma_y-1)/sigma_y) * ( (mu_@{j}) * exp(q_@{j})/exp(y_@{j}) )^(1/sigma_q) * ( (1-alpha_@{j}) * exp(y_@{j})/exp(l_@{j}) )^(1/sigma_y);
