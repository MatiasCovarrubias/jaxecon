"""
Model-Specific Tables Module for RbcProdNet

NOTE: For general analysis tables (descriptive stats, welfare, stochastic SS),
use the functions in DEQN/analysis/tables.py instead.

This module should contain only model-specific table generation functions
that require specific knowledge of the RbcProdNet model structure.
"""
...
# ============================================================================
# DEPRECATED: Use DEQN/analysis/tables.py instead
# The functions below are kept for backward compatibility but should not be used
# for new analysis. Import from DEQN.analysis.tables instead.
# ============================================================================
...
def create_descriptive_stats_table(
    ...
    for exp_name, analysis_vars_dict in analysis_variables_data.items():
        # Calculate descriptive statistics for each analysis variable
...
def _generate_latex_table(df: pd.DataFrame) -> str:
    ...
    for index, row in df.iterrows():
        ...
        if any(keyword in str(index) for keyword in ["Intratemporal Utility"]):
...
def _generate_stochastic_ss_latex_table(stochastic_ss_data: Dict[str, Dict[str, float]]) -> str:
    ...
    -----------
    stochastic_ss_data : dict
        Dictionary with experiment names as keys and dictionaries of variable labels to values
