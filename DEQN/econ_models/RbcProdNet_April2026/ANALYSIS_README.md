...
- compute stochastic SS, IR/GIR, and welfare objects only when their corresponding module switches are enabled
- automatically enable stochastic SS when stochastic-steady-state IRs are requested
...
- Python also exports and prints a CIR analysis table under `IRs/IR_tables`, plus one CIR shock-size profile figure per table measure under `IRs/IR_CIR`. The table reports amplification, attenuation, and asymmetry in percent, and reports sectoral correlations as unitless coefficients.
- In the combined LaTeX wrapper, aggregate histograms are grouped as consumption, GDP, capital, and investment in a paper-oriented two-by-two figure. The standalone histogram PNGs are still saved for reuse.
- Only the consumption histogram carries the shared legend; all histogram panels use enlarged axis and tick fonts for the two-column layout.
- With `long_simulation = true`, aggregate histogram FirstOrder curves use a matched long simulation with the same seeds, initial states, shock realizations, burn-in, and retained periods as the global solution. This run is independent of `run_welfare`.
...
- Shock sizes are no longer meant to be hardcoded in the main config. Python discovers them from `ModelData_IRs.mat` or the legacy MATLAB IR directory, stores the discovered list back into `config["ir_shock_sizes"]`, and formats shock keys canonically with underscores for decimals, such as `pos_12_5`.
- If benchmark IR objects are unavailable, set `ir_shock_sizes` explicitly to a list of percentages. Values must be finite and strictly between 0 and 100. The old scalar `shock_size` setting was unused by this pipeline and has been removed.
...
- `config["long_simulation"]` selects the main nonlinear reporting sample. `False` replays the exact shock sequence and dates stored in `ModelData_simulation.mat`, allowing DEQN and MATLAB methods to be compared under identical realized shocks. `True` draws a long random DEQN simulation used for ergodic distributions. When `long_simulation = false` and `ergodic_price_aggregation = true`, Python also runs an auxiliary long ergodic reference sample for fixed-price weights, GIR averaging, stochastic SS, histograms, and ergodic-only sectoral outputs. When `long_simulation = true`, Python also computes a matched long first-order log-linear simulation from the Dynare state-space matrices for histogram and other simulation outputs; its utilities are retained only when welfare is enabled.
...
- `experiments_to_analyze` is the only experiment selector. Set it to a checkpoint string such as `"finercal_July"`, or to a single-entry mapping such as `{"benchmark": "finercal_July"}` when a custom display label is needed.
...
- `run_stochastic_ss`, `run_impulse_responses`, and `run_welfare` control the three expensive optional modules. All default to `true`. If `run_impulse_responses = true` and `use_gir = false`, stochastic SS is enabled automatically because that initial state is required. GIRs do not require a stochastic steady state.
- The default IR benchmark overlays are `["PerfectForesight", "FirstOrder"]`, displayed as MIT shock and 1st-order approximation. The config still accepts `ir_benchmark_methods` to override the set or ordering, and still accepts the legacy single-string `ir_benchmark_method` for backward compatibility.
...
- sectoral stochastic-SS composition-share bars
- sectoral ergodic-mean bars
- sectoral ergodic-mean composition-share bars
...
| `\input{IRs/IR_tables/cir_analysis_basefinal.tex}`                                                                                    | `analysis/basefinal/IRs/IR_tables/cir_analysis_basefinal.tex`           | `analysis_hooks.py` → `_write_cir_analysis_table(...)`                                                     | This function writes the CIR analysis table layout, panel titles, row labels, formulas, and note text.                                                                                                                      |
| `\input{IRs/IR_tables/cir_regressions_basefinal.tex}`                                                                                 | `analysis/basefinal/IRs/IR_tables/cir_regressions_basefinal.tex`        | `analysis_hooks.py` → `_write_cir_regression_table(...)`                                                  | Cross-sector OLS table: Panels A--B are CIR outcomes by shock size; Panel C is ergodic capital share change.                                                                                                              |
| CIR shock-size profile figures                                                                                                        | `analysis/basefinal/IRs/IR_CIR/*.png`                                   | `analysis_hooks.py` → `render_cir_analysis_outputs(...)` and `plot_helpers.py` → `plot_cir_shock_size_profiles(...)` | The hook builds the CIR rows and selects which measures to plot; the plotting helper renders shock size on the x axis and table measures on the y axis.                                                                      |
| Aggregate histogram figure captions and ordering                                                                                     | `simulation/aggregate_histograms_basefinal.tex` and `figures_tables_basefinal.tex` | `analysis_hooks.py` → `_write_aggregate_histogram_figure_tex(...)` and `get_report_sections(...)` | The hook writes the reusable four-panel fragment and groups the same consumption, GDP, capital, and investment PNGs after descriptive statistics in the master output. |
...
| Which figures appear, in what order                                                                                                  | `figures_tables_basefinal.tex`                                          | `DEQN/analysis.py` → `_build_analysis_latex_sections(...)`                                                 | This is where the wrapper decides the sequence: model-vs-data table, aggregate IRs, grouped paper-ready aggregate IRs for the largest negative shock, grouped appendix aggregate IRs for the largest negative shock, stochastic-SS sectoral figures, stochastic-SS sectoral composition figures, aggregate stochastic-SS table, descriptive stats, aggregate histograms, welfare, sectoral IRs, ergodic sectoral figures, and ergodic sectoral composition figures. |
...
6. sectoral composition-share changes in stochastic SS
7. aggregate stochastic SS
8. descriptive statistics
9. aggregate histograms
10. welfare costs
11. sectoral IRs
