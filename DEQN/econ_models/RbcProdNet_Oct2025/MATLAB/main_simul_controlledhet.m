...
params.rho = modrho_mean * ones(dim,1);
params.rho = 0.85 * ones(dim,1);

[Qv, Dv] = eig(modvcv);
eigenvalues=diag(Dv);
eigen_mean = va_data*eigenvalues/sum(va_data);
diag_matrix1= diag(eigen_mean * ones(37,1));

num_simulations = 1000;
simulated_data = mvnrnd(zeros(1, dim), modvcv, num_simulations);
std_devs = std(simulated_data)';
std_devs_mean = 1.5*va_data*std_devs/sum(va_data);
disp(std_devs_mean^2)
diag_matrix = diag(std_devs_mean^2 * ones(37,1));
params.Sigma_A = diag_matrix1;
