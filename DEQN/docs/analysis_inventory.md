...
This file is an execution inventory: what `DEQN/analysis.py` actually runs for `RbcProdNet_March2026`, in the order it is displayed.

For concise Python-side documentation of the analysis codebase, see:

- `DEQN/econ_models/RbcProdNet_March2026/ANALYSIS_README.md`

For the MATLAB-side pipeline and saved-object contract, see:

- `DEQN/econ_models/RbcProdNet_March2026/MATLAB/CODEBASE_README.md`
- `DEQN/econ_models/RbcProdNet_March2026/MATLAB/ModelData_README.md`

Output roots:

- `<model_dir>/analysis/<analysis_name>/` for top-level tables and `config.json`
- `<model_dir>/analysis/<analysis_name>/simulation/` for simulation-related TeX and sectoral plots
- `<model_dir>/analysis/<analysis_name>/IRs/` for IR figures
...
- In code, aggregate stochastic SS tables (4) come before descriptive statistics (5).
...
- Loads MATLAB simulation artifacts from `ModelData_simulation.mat`. If only `FirstOrder` exists in that object, only the log-linear benchmark can appear in simulation-based descriptive tables.
...
- Carries the analysis rule that non-comparative sectoral stochastic-SS and GIR exercises should use the long ergodic run.
